...
class BetaCalibration(BaseEstimator, RegressorMixin):
    ...
    def __init__(self, scores, ground_truth, score_min=-1, score_max=1):
        ...
        X = column_or_1d(self.normalize(scores, score_min=score_min, score_max=score_max))
        y = column_or_1d(ground_truth)
        X, y = indexable(X, y)
        self.map_, self.lr_ = self._beta_calibration(X, y, )
    
    def normalize(self, score, score_min=-1, score_max=1):
    ...
    def predict(self, S):
        ...
        """
        df = column_or_1d(self.normalize(S, score_min=self.score_min, score_max=self.score_max)).reshape(-1, 1)
    ...
    def _beta_calibration(self, df, y, sample_weight=None):

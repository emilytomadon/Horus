...
def process_images(base_directory, model, csv_filename, total_comparisons):
    ...
    for cluster_id in range(num_clusters):
        ...
        # Compare images with the same 7-digit prefixes
        for img1, img2 in sampled_pairs:
            if img1.stem[:7] == img2.stem[:7]:  # Compare only if the first 7 digits are the same
                distance = compare_images(img1, img2, model)
                results.append([img1.name, img2.name, distance, 1, cluster_id])
...
if __name__ == "__main__":
    ...
    csv_filename = 'C:/Users/emily/OneDrive/Documents/IC/resultados/elasticface/clusteringTrue.csv'

# used Jupyter Notebook to work with the architecture alongside other codesimport torch
from torch import nn

# Define the IResNet100 model
...
class IResNet(nn.Module):
    ...
    def _make_layer(self, block, planes, blocks, stride=1, dilate=False, use_se=False):
        ...
        layers = []
        layers.append(
            block(self.inplanes, planes, stride, downsample, self.groups, self.base_width, previous_dilation, use_se=use_se))
        self.inplanes = planes * block.expansion
        for _ in range(1, blocks):
            layers.append(
                block(self.inplanes, planes, groups=self.groups, base_width=self.base_width, dilation=self.dilation, use_se=use_se))
...
# Load weights from file with map_location for CPU
checkpoint_path = 'C:\\Users\\emily\\Downloads\\295672backbone.pth'
...
print(y.size())  # Should be (1, 512) if num_features is 512

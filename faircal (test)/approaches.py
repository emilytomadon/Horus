...
def cluster_methods(nbins, calibration_method, dataset_name, feature, fold, db_fold, n_clusters,
    ...
    if not os.path.exists(saveto):
        ...
        print("Embeddings shape:", embeddings.shape if embeddings is not None else "None")
        kmeans = KMeans(n_clusters=n_clusters)
        try:
            kmeans.fit(embeddings)
            np.save(saveto, kmeans)
        except Exception as e:
            print("Error during KMeans fitting or saving:", str(e))
    else:
        max_wait = 100
        attempts = 0
        ...
        if attempts == max_wait:
            raise RuntimeError("KMeans computation timeout.")

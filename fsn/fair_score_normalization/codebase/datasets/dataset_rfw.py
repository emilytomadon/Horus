...
class RFWDataset(Dataset):
    ...
    def __init__(self,
                 embedding_type,
                 folds:List[int],
                 data_path:str="/home/gimicaroni/Documents/Datasets/rfw_embeddings/rfw_train"):
        ...
        cv_mask = np.load('cv-mask-rfw.npy')

        # Create 50 random embeddings with size 128 (just like facenet)
        embeddings_dir = '/home/gimicaroni/Documents/Datasets/rfw_embeddings/rfw_train'
        embeddings_dirlist = os.listdir(embeddings_dir)
        embeddings = [f for f in embeddings_dirlist if f.endswith('.npy')]
        embeddings_list = [np.load(os.path.join(embeddings_dir, f)) for f in embeddings]
        embeddings_array = np.vstack(embeddings_list)

        self._embeddings = embeddings_array

        # Only one sample per id
        self._ids = np.load('ids-rfw-train.npy')

        self._filenames = None # Path to image per sample

        features_ethnicity_array = np.array([int(f[0]) for f in embeddings_dirlist if f.endswith('.npy')])
        np.save('ethnicity-array-rfw.npy', features_ethnicity_array)
        self._features_ethnicity = features_ethnicity_array
        ...
        self._feature_constructor = namedtuple("ethnicity",
                                               ["ethnicity"])
        
        # Create a namedtuple constructor for an Example-Item
        self._item_constructor = namedtuple("EmbeddingItem", ["id",
                                                            "embedding",
                                                            "embedding_type",
                                                            "filename",
                                                            "feature"
    ...
    def generate_cv_mask(num_folds,
        ...
        pu.file_np_save(f"cv-mask-rfw.npy", mask)
